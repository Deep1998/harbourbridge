...
import (
	...
	"github.com/cloudspannerecosystem/harbourbridge/spanner/batchwriter"
...
func DataConv(ctx context.Context, sourceProfile profiles.SourceProfile, targetProfile profiles.TargetProfile, ioHelper *utils.IOStreams, client *sp.Client, conv *internal.Conv, dataOnly bool) (*batchwriter.BatchWriter, error) {
	config := batchwriter.BatchWriterConfig{
...
func dataFromDatabase(sourceProfile profiles.SourceProfile, config batchwriter.BatchWriterConfig, client *sp.Client, conv *internal.Conv) (*batchwriter.BatchWriter, error) {
	infoSchema, err := GetInfoSchema(sourceProfile)
	if err != nil {
		return nil, err
	}
	...
	writer := batchwriter.NewBatchWriter(config)
	conv.SetDataMode()
	conv.SetDataSink(
...
func dataFromDump(driver string, config batchwriter.BatchWriterConfig, ioHelper *utils.IOStreams, client *sp.Client, conv *internal.Conv, dataOnly bool) (*batchwriter.BatchWriter, error) {
...
func dataFromCSV(ctx context.Context, sourceProfile profiles.SourceProfile, targetProfile profiles.TargetProfile, config batchwriter.BatchWriterConfig, conv *internal.Conv, client *sp.Client) (*batchwriter.BatchWriter, error) {
...
// to file 'name'.
func WriteBadData(bw *batchwriter.BatchWriter, conv *internal.Conv, banner, name string, out *os.File) {
