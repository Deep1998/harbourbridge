...
func (p *PrepareDataflowWriter) Transaction(ctx context.Context) error {
	...
	dfLaunchReq, err := dataflowutils.GetDataflowLaunchRequest(params, writerTuningCfg)
	if err != nil {
		return err
	}
...
func validateUpdateWriterTuningCfg(cfg *dataflowaccessor.DataflowTuningConfig, spannerProjectId, spannerLocation, smtJobId string) {
	if cfg.ProjectId == "" {
